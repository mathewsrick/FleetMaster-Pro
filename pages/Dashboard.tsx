...
const Dashboard: React.FC = () => {
  ...
  if (loading) return <div className="flex items-center justify-center h-full text-indigo-600"><i className="fa-solid fa-circle-notch fa-spin text-4xl"></i></div>;
    ...
    <div className="space-y-6">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Tooltip cursor={{stroke: '#6366f1', strokeWidth: 1}} contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
        ...
        <div className="space-y-6">
          <div className="bg-indigo-600 p-6 rounded-xl shadow-lg shadow-indigo-200 text-white">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm"><span className="opacity-80">Potencial Semanal</span><span className="font-bold">${stats.potentialRevenue.toLocaleString()}</span></div>
              ...
              <div className="text-xs opacity-80 italic">* {stats.totalVehicles > 0 ? Math.round((stats.activeVehicles / stats.totalVehicles) * 100) : 0}% de los vehículos rentados.</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div><h2 className="text-lg font-bold text-slate-800">Explorador de Vehículos</h2><p className="text-sm text-slate-500">Métricas individuales</p></div>
          <select value={selectedVehicleId} onChange={(e) => setSelectedVehicleId(e.target.value)} className="w-full sm:w-64 px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-medium">
        ...
        {selectedVehicle && vehicleStats ? (
          ...
          <div className="p-12 text-center text-slate-400"><i className="fa-solid fa-magnifying-glass-chart text-4xl mb-3 opacity-20"></i><p>Seleccione un vehículo.</p></div>
        )}
      </div>
    </div>
  );
...
const StatCard: React.FC<{ title: string, value: string, icon: string, color: string }> = ({ title, value, icon, color }) => {
  const colorMap: Record<string, string> = { indigo: 'bg-indigo-50 text-indigo-600', green: 'bg-emerald-50 text-emerald-600', rose: 'bg-rose-50 text-rose-600', blue: 'bg-sky-50 text-sky-600' };
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-5">
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl ${colorMap[color]}`}><i className={`fa-solid ${icon}`}></i></div>
      <div><p className="text-sm font-medium text-slate-500">{title}</p><h3 className="text-2xl font-bold text-slate-900">{value}</h3></div>
    </div>
  );
};
