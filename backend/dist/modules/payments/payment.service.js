...
export const getByDriver = async (userId, driverId) => await paymentRepo.findByDriver(userId, driverId);
export const create = async (userId, data) => {
    const { generateArrear = true, ...paymentData } = data;
    const payment = {
    ...
    const pendingArrears = allPendingArrears.filter((a) => a.status === 'pending');
    const totalAccumulatedDebt = pendingArrears.reduce((sum, a) => sum + Number(a.amountOwed), 0);
    const driver = await driverRepo.findById(userId, payment.driverId);
    if (driver && driver.email) {
        await emailService.sendEmail({
            to: driver.email,
            subject: `Recibo de Pago - FleetMaster Hub - $${payment.amount.toLocaleString()}`,
            html: emailService.templates.paymentConfirmation(payment.amount, payment.date, payment.type, createdArrearAmount, totalAccumulatedDebt, pendingArrears)
        });
    }
