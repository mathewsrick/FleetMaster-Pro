...
export const assignDriverToVehicle = async (userId, driverId, vehicleId) => {
    // 1. Obtener datos del conductor
    const driver = await driverRepo.findById(userId, driverId);
    if (!driver) {
    ...
    if (vehicleId) {
        ...
        if (driver.email && updatedDriver.vehicle) {
            await emailService.sendEmail({
                ...
                html: emailService.templates.vehicleAssignment(`${driver.firstName} ${driver.lastName}`, updatedDriver.vehicle)
            });
        }
    }
    return { success: true };
